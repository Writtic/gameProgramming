...
public:
  ...
  State(int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9) {
    num[0] = x1; num[1] = x2; num[2] = x3; num[3] = x4; num[4] = x5; num[5] = x6;
    num[6] = x7; num[7] = x8; num[8] = x9;
...
{
  for (std::list<State>::const_iterator it = list.begin(); it != list.end();
       ++it) {
    if (s.IsSame(*it) == true) return true;
  }
  return false;
}
...
{
  for (std::vector<State>::const_iterator it = list.begin(); it != list.end();
       ++it) {
    if (s.IsSame(*it) == true) return true;
  }
  return false;
...
int main()
{
  ...
  while (true)
  {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (IsInList(next, OPEN) == true) continue;

      if (IsInList(next, CLOSE) == true) continue;
