...
public:
  ...
  void f()
  { x++; y++; }
  // 변수 수정을 해당 함수에서 할 수 없음. 있는 값 그대로 써야함.
  // 물론 다른 함수에서 수정된 값이 반영되긴 함.
  void Print() const {
    cout << x << ',' << y << endl;
  }
...
int main()
{
  ...
  // 그럼 프로그래머는 6을 대입했을 때 Point(6, 0)을 허용할지 안할지는...
  // 옳고 그름이 없음. 우리가 정의해야함.
  // 허용하지 않으려면 explicit!
  // 근데 explicit을 안 쓰면 코드가 폭망하는 경우가 생김
  // (위의 경우는 버그로 인식되지 않음, 디버깅 불가)
  // explicit은 암시적 형변환을 못하게 막음.
  // (컴파일시 에러로 인식! 더 책임감을 가지고 짤수밖에 없음!)
}
